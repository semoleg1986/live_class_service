...
export default (): { liveClass: LiveClassConfig } => ({
  liveClass: {
    ...
    maxParticipants: parseIntOrDefault(
      process.env.LIVE_CLASS_MAX_PARTICIPANTS,
      11,
    ),
    ...
    outboxBatchSize: parseIntOrDefault(
      process.env.LIVE_CLASS_OUTBOX_BATCH_SIZE,
      100,
    ),
    outboxPollIntervalMs: parseIntOrDefault(
      process.env.LIVE_CLASS_OUTBOX_POLL_INTERVAL_MS,
      1000,
    ),
    databaseUrl:
